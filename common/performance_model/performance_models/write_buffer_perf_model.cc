...
WriteBufferPerfModel::WriteBufferPerfModel(UInt32 num_entries, bool coalescing, const SubsecondTime& insertion_latency) :
...
void
WriteBufferPerfModel::consume(const SubsecondTime& t_now)
{
...
SubsecondTime
WriteBufferPerfModel::getInsertionLatency(IntPtr address, const SubsecondTime& send_latency, ShmemPerfModel* perf)
{
...
SubsecondTime
WriteBufferPerfModel::flush(ShmemPerfModel* perf)
{
   return m_queue.front().second - perf->getElapsedTime(ShmemPerfModel::_USER_THREAD);
}

SubsecondTime
WriteBufferPerfModel::flushAll(ShmemPerfModel* perf)
{
