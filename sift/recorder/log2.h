...
class LogMessage {
    ...
    inline std::string Demangle(char const *msg_str) {
        ...
        if (demangled_symbol && status == 0 && length > 0) {
            string symbol_str(demangled_symbol );
...
#define LOG2(TYPE) LOG2_##TYPE
#define LOG2_FATAL LogMessageFatal(__FILE__,__LINE__).stream()
#define LOG2_INFO LogMessage(__FILE__,__LINE__).stream()
