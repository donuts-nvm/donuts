...
NvmPerfModelReadWrite::NvmPerfModelReadWrite(const core_id_t core_id, const UInt32 cache_block_size) :
    NvmPerfModel(core_id, cache_block_size),
    m_queue_model_read(nullptr),
    m_queue_model_write(nullptr),
    m_nvm_read_cost(loadReadLatency()),
    m_nvm_write_cost(loadWriteLatency()),
    ...
    m_total_write_queueing_delay(SubsecondTime::Zero())
{
   ...
   {
      const String queue_model_type       = Sim()->getCfg()->getString("perf_model/dram/queue_model/type");
      const SubsecondTime rounded_latency = m_nvm_bandwidth.getRoundedLatency(8 * cache_block_size);// bytes to bits
...
SubsecondTime
NvmPerfModelReadWrite::computeQueueDelay(SubsecondTime pkt_time, SubsecondTime processing_time, const core_id_t requester,
...
void
NvmPerfModelReadWrite::increaseQueueDelay(const DramCntlrInterface::access_t access_type, SubsecondTime queue_delay)
{
   if (access_type == DramCntlrInterface::READ)
