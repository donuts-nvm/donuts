...
{
   const auto min_value = 1 / associativity;
   LOG_ASSERT_ERROR(m_threshold >= 1 && m_threshold <= associativity,
                    "The associativity set threshold must ensure at least one cache block and less than associativity. "
                    "For a cache of associativity = %u, the min-value is %.1f (%.1f%%) and max-value is 1.0 (100%%), %u blocks",
                    min_value, min_value * 100, associativity);
}
