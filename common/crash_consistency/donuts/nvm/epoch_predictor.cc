...
EpochPredictorCache::EpochPredictorCache(const UInt32 num_sets,
                                         const UInt32 associativity,
                                         const String& replacement_policy) :
    Cache("EpochPredictorCache", num_sets, associativity, 1, replacement_policy,
          SHARED_CACHE, HASH_MASK, nullptr, nullptr,
          [&]()
          {
             const auto policy = CacheSet::parsePolicyType(replacement_policy);
             m_sets            = new CacheSet*[num_sets];
             m_set_info        = CacheSet::createCacheSetInfo("EpochPredictorCache", "perf_model/epoch_predictor",
                                                              0, policy, associativity);
             for (UInt32 i = 0; i < num_sets; i++)
             {
                m_sets[i] = CacheSet::createCacheSet(policy, SHARED_CACHE, associativity, sizeof(IntPtr), m_set_info);
             }
          }) {}
...
EpochPredictor::EpochPredictor() :
    m_cache(getNumSets(), getAssociativity(), getReplacementPolicy()),
...
EpochPredictor::getDefaultMode()
{
   ...
   const auto default_policy = LogPolicy::getType(value);
