...
def is_donuts(config):
    return get_project_type(config) == 'donuts'

def is_picl(config):
    return get_project_type(config) == 'picl'


def get_project_type(config):
    key = 'general/project_type'
    return config[key] if key in config else 'baseline'
...
def get_num_buffer_overflow(res):
    ...
    mem_log_key = {
        'donuts': 'dram.log_row_buffer_overflow',
