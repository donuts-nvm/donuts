...
#define MYLOG(...)                                                                                                                                                                                                    \
   {                                                                                                                                                                                                                  \
      ScopedLock l(iolock);                                                                                                                                                                                           \
      fflush(stdout);                                                                                                                                                                                                 \
      printf("[%s] %d%cdr %-25s@%3u: ", itostr(getShmemPerfModel()->getElapsedTime()).c_str(), getMemoryManager()->getCore()->getId(), Sim()->getCoreManager()->amiUserThread() ? '^' : '_', __FUNCTION__, __LINE__); \
      printf(__VA_ARGS__);                                                                                                                                                                                            \
      printf("\n");                                                                                                                                                                                                   \
      fflush(stdout);                                                                                                                                                                                                 \
   }
...
boost::tuple<SubsecondTime, HitWhere::where_t>
NvmCntlr::logDataToNvm(IntPtr address, const core_id_t requester, Byte* data_buf, const SubsecondTime& now)
{
   if (Sim()->getFaultinjectionManager())
   {
      if (m_log_map[address] == nullptr)
      {
         LOG_PRINT_ERROR("Log Buffer does not exist");
      }
