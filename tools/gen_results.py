...
def parse_args():
    ...
    parser.add_argument('-i', '--info', type=str, nargs='+', choices=COLUMN_OPTIONS, default=COLUMN_OPTIONS,
                        help='information colums')
...
def find_configs(root_dir, app_names):
    ...
    for app_name in app_names:
        ...
        try:
            app_configs = list(
                filter(lambda f: os.path.isdir(f"{app_dir}/{f}"), sorted(os.listdir(app_dir), key=natural_keys)))
...
def get_args(args):
    ...
    app_names = args.app if args.app else list(filter(lambda f:
                                                      os.path.isdir(f"{root_dir}/{f}"),
                                                      sorted(os.listdir(root_dir), key=natural_keys)))
    configs = args.config if args.config else find_configs(root_dir, app_names)
    test_name = '_' + args.test.replace('/', '-') if args.test else ''
    out_file = f"{root_dir}/results_{test_name}_{args.benchmark}.xlsx"
